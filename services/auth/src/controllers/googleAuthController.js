...
import { getJWTSecret } from '@skillsync/shared/jwt';
...
export async function postGoogleAuth(req, res) {
  try {
    ...
    if (!user) {
      const legacy = await User.findOne({ email, $or: [{ googleId: { $exists: false } }, { googleId: null }] });
      if (legacy) {
  ...
  try {
    ...
    if (dup) {
      return res.status(409).json({
        error: 'An account with this email already exists. Sign in with Google from that account.',
      });
    }
